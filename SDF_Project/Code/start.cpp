...
void welcome(int n)
{
    std::cout<<"Welcome all the "<<n<<" players"<<std::endl;
}
...
{
    sf::RenderWindow window(sf::VideoMode(400, 100), "Messasge", sf::Style::Close);
...
Inputs Run(bool& run)
{
    ...
    sf::Texture next;
    if(next.loadFromFile("../Images/next.png") == false)
        std::cout << "Loading failed" << std::endl;
    ...
    sf::Sprite button1;
    setSprite(button1, 0.6, 0.3, 530, 720, &next);
    ...
    sf::Sprite banner;
    setSprite(banner, 1.1, 0.8, 100, -50, &Banner);

    sf::Texture two_players, three_players, four_players;
    if (two_players.loadFromFile("../Images/2_players.png") == false)
        std::cout << "Loading failed" << std::endl;

    if (three_players.loadFromFile("../Images/3_players.png") == false)
        std::cout << "Loading failed" << std::endl;

    if (four_players.loadFromFile("../Images/4_players.png") == false)
        std::cout << "Loading failed" << std::endl;
                ...
                if (button1.getGlobalBounds().contains(sf::Mouse::getPosition(window1).x, sf::Mouse::getPosition(window1).y))
                {
                    ...
                    else ///################## Opening the next window ################
                    {
                        ...
                        while (window3.isOpen())
                        {
                            ...
                            sf::Texture start;
                            if (start.loadFromFile("../Images/start_new.png") == false)
                                std::cout << "Loading failed" << std::endl;

                            sf::Sprite Start;
                            setSprite(Start, 0.6, 0.3, 140, 450, &start);
                            ...
                            while (window3.pollEvent(event3))
                            {
                                ...
                                if (event3.type == sf::Event::MouseButtonPressed)
                                {
                                    ...
                                    if (Start.getGlobalBounds().contains(sf::Mouse::getPosition(window3).x, sf::Mouse::getPosition(window3).y))
                                    {
                                        int flag=0;
                                        if ((str1 == "\0") || (str1 == "_") || (str2 == "\0") || (str2 == "_") || (str3 == "\0") || (str3 == "_")||str4 == "\0" || str4 == "_")
                                        {

                                            if (str3 == "\0" || str3 == "_")
                                            {
                                                if (three.getOutlineColor() == sf::Color(255, 0, 0) || four.getOutlineColor() == sf::Color(255, 0, 0))
                                                {
                                            ...
                                            else if (str1 == "\0" || str1 == "_" || str2 == "\0" || str2 == "_")
                                                {
                                                    msgDisplay("Enter all players' names");
                                                    flag=1;
                                                }
